...
- 手动 `/boxbox` 随时评估；会话中仅**质量负信号**（连续纠正 ≥2 / 同工具连失败 ≥2 / 明确要求换模）自动弹建议，确认后才换，绝不静默。
...
✅ 会话中自动建议（仅质量负信号触发：连续纠正 ≥2 / 同工具连失败 ≥2 / 明确要求换模；确认后才换，绝不静默）
✅ 打标时参考内置市面模型表 `src/builtin-models.json`（`power` 越大越强，`deprecated: true` 默认跳过）
✅ 换模成本评估（含 prompt cache 丢失说明）

❌ 自动静默换模（始终需要确认）
❌ 会话中自动降档省耗（省耗只在手动 `/boxbox` 时评估）
❌ 轮次/上下文/时长等体量信号单独触发自动建议（只升 heat，供评判参考）
...
sequenceDiagram
  ...
  F-->>U: 模式 + 理由 + 成本带
  U->>Pi: 确认 → setModel
  Note over F: 会话中检测到质量负信号（连续纠正/工具失败/要求换模）时走同一条建议链路自动触发
```
